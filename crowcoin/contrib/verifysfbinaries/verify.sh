...
###   This script attempts to download the signature file SHA256SUMS.asc from crowcoin.org
...
WORKINGDIR="/tmp/crowcoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://crowcoin.org/bin/"
VERSIONPREFIX="crowcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'crowcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Crowcoin Core binary release signing key installed?"
   fi
