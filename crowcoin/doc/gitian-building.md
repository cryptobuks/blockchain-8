...
*Setup instructions for a Gitian build of Crowcoin using a Debian VM or physical system.*
...
- [Building Crowcoin](#building-crowcoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-crowcoin-host/README.md).
...
Clone the git repositories for crowcoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/crowcoin/crowcoin
```
...
in the crowcoin repository under 'Fetch and build inputs' to install sources which require
...
To build Crowcoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the crowcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/crowcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/crowcoin/crowcoin
...
For example:
```bash
URL=https://github.com/laanwj/crowcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit crowcoin=${COMMIT} --url crowcoin=${URL} ../crowcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit crowcoin=${COMMIT} --url crowcoin=${URL} ../crowcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit crowcoin=${COMMIT} --url crowcoin=${URL} ../crowcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the crowcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../crowcoin/contrib/gitian-descriptors/*|sort|uniq )
...
GCCPATH=/some/root/path/crowcoin.git
SIGPATH=/some/root/path/crowcoin-detached-sigs.git

./bin/gbuild --url crowcoin=${GCCPATH},signature=${SIGPATH} ../crowcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/crowcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/crowcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/crowcoin-osx-build.assert
```
...
[crowcoin/gitian.sigs](https://github.com/crowcoin/gitian.sigs/) repository, or if that's not possible create a pull
