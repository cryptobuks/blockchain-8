 ...
 *
 * This is the developer documentation of the reference client for an experimental new digital currency called Crowcoin (https://www.crowcoin.org/),
 * which enables instant payments to anyone, anywhere in the world. Crowcoin uses peer-to-peer technology to operate
...
bool AppInit(int argc, char* argv[]) // 3.0.应用程序初始化
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/crowcoin.conf are parsed in qt/crowcoin.cpp's main()
    ...
    {
        std::string strUsage = _("Crowcoin Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  crowcoind [options]                     " + _("Start Crowcoin Core Daemon") + "\n";

            strUsage += "\n" + HelpMessage(HMM_CROWCOIND); // 帮助信息
        }
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) // 3.6.检测每个命令行参数是否以'-'或'/'开头
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "crowcoin:"))
                fCommandLine = true;

        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in crowcoind anymore. Use the crowcoin-cli utility instead.\n");
            exit(1);
        }
#ifndef WIN32
...
int main(int argc, char* argv[]) // 0.程序入口
{
    ...
    // Connect crowcoind signal handlers
